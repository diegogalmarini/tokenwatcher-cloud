...
export default function Navbar() {
  ...
  // Para evitar el error de hidratación, no renderizamos nada hasta que el componente esté montado en el cliente
  if (!mounted) {
    // Puedes retornar un skeleton/loader aquí si lo prefieres
    return (
        <header className="sticky top-0 left-0 w-full z-50 bg-white dark:bg-[#262626] shadow-md">
            <div className="max-w-screen-xl mx-auto flex items-center justify-between px-4 sm:px-6 py-3 h-[68px]">
                {/* Placeholder para el logo para evitar saltos de layout */}
    ...
    <header className={`sticky top-0 left-0 w-full z-50 transition-colors ${isDark ? "bg-[#262626]" : "bg-white"} shadow-md`}>
      <div className="max-w-screen-xl mx-auto flex items-center justify-between px-4 sm:px-6 py-3">
        ...
        <nav className="hidden lg:flex items-center space-x-4">
          ...
          {/* --- CORRECCIÓN: Eliminado 'legacyBehavior' --- */}
          <Link href="/register">
