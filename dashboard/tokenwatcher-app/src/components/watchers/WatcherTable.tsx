...
export default function WatcherTable({ watchers, onEdit, onDelete, onToggleActive }: Props) {
  if (!watchers || watchers.length === 0) {
    return <p className="text-center text-gray-500 dark:text-gray-400 py-8">No watchers yet. Create one to get started!</p>; // English
  }

  const shortenAddress = (address: string) => {
      if (!address) return 'N/A'; // English
              ...
              <tr key={watcher.id} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                ...
                <td className="px-4 py-3 whitespace-nowrap max-w-xs">
                  <a
                    ...
                    title={watcher.webhook_url || "No webhook configured"} // English
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    {watcher.webhook_url ? `${watcher.webhook_url.slice(0, 35)}...` : "N/A"} {/* English */}
                  </a>
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    ...
                    {watcher.is_active ? "Active" : "Paused"} {/* English */}
                  </span>
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-sm font-medium space-x-2">
                  ...
                  >
                    {watcher.is_active ? "Pause" : "Activate"} {/* English */}
                  </Button>
                  <Button
                    intent="destructive"
                    size="sm"
