...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    // Capturamos ?token=xxxx de la URL
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("success");
      // Opcional: redirigir a login luego de 2 segundos
      setTimeout(() => {
        router.push("/login");
      }, 2000);
    } catch (err: any) {
        ...
        <div className="w-full max-w-md">
          ...
          {status === "success" ? (
            <div
              className={`mb-4 ${
                isDark
                  ? "text-gray-100 bg-green-800 p-4 rounded"
                  : "text-gray-900 bg-green-100 p-4 rounded"
              }`}
            >
